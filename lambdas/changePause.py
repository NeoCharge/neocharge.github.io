...
def lambda_handler(event, context):
    ...
    with connection.cursor() as cur:
        cur.execute('select DevId, Pause from Users where Email=%(userEmail)s', { 'userEmail': email,})
    ...
    if pause_success : 
        with connection.cursor() as cur:
            ...
            cur.execute('Update Users set Pause=%(setPause)s where Email=%(userEmail)s',
                {'setPause' : (not is_cur_paused), 'userEmail': email, })
            cur.execute('SET SQL_SAFE_UPDATES = 1;')

    return {
        'statusCode': 200,
        'headers': { 'Content-Type': 'json' },
        'body': {"success" : pause_success, "changedTo" :(not is_cur_paused)}
    }
